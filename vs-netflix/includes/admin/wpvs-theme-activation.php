...
class WPVS_THEME_ACTIVATION_MANAGER {
    public function __construct() {
        if( ! current_user_can('manage_options') ) {
            return;
        }

        add_action( 'wp_ajax_wpvs_theme_activate_customer_access', array($this, 'wpvs_theme_activate_customer_access') );
        add_action( 'wp_ajax_wpvs_theme_deactivate_customer_access', array($this,'wpvs_theme_deactivate_customer_access') );
    ...
    public function wpvs_theme_activate_customer_access() {
        if( current_user_can('manage_options') ) {
            if ( isset($_POST['email']) && ! empty($_POST['email']) && isset($_POST['license_key']) && !empty($_POST['license_key']) ) {
                ...
                if( isset($wpvs_license_key_data['license_key_is_valid']) && $wpvs_license_key_data['license_key_is_valid'] ) {
                    ...
                    if( isset($wpvs_license_key_data['license_key_product']) ) {
                        ...
                        if( $wpvs_license_key_type == 'theme' && $wpvs_license_key_data['license_key_product'] != $wpvs_this_theme_product_slug ) {
                            $error_message = __('That license key is not for this theme', 'wpvs-theme');
                        ...
                        if( $wpvs_license_key_type == 'plugin' && $wpvs_license_key_data['license_key_product'] != 'vimeo-sync-memberships' ) {
                            $error_message = __('That license key is not for the WP Video Memberships plugin.', 'wpvs-theme');
                            $this->handle_ajax_error($error_message, 400);
                        }
                    }
                    ...
                    if( $wpvs_license_key_type == 'theme' ) {
                        update_option('wpvs_account_theme_license_key', $new_license_key_data);
                    }
                    ...
                    if( ! empty($wpvs_license_key_data['error']) ) {
                        if($wpvs_license_key_data['error'] == "site_activation_failed") {
                            $error_message = __('Unable to activate your website. Make sure this license key has not already been activated on another URL', 'wpvs-theme');
                        }

                        if($wpvs_license_key_data['error'] == "user_not_found") {
                            $error_message = __('A user with that Email does not exist', 'wpvs-theme');
                        }
                        ...
                        $error_message = __('Something went wrong activating your website', 'wpvs-theme');
                    }
                    $this->handle_ajax_error($error_message, 400);
                }
            }
    ...
    public function wpvs_theme_deactivate_customer_access() {
        if ( isset($_POST['email']) && !empty($_POST['email']) && isset($_POST['license_key']) && !empty($_POST['license_key']) ) {
            $wpvs_user_email = urlencode($_POST['email']);
            $wpvs_license_key = urlencode($_POST['license_key']);
            $wpvs_license_key_type = urlencode($_POST['license_type']);
            ...
            if( $wpvs_license_key_type == 'theme' ) {
                update_option('wpvs_account_theme_license_key', $update_license_key_data);
            }
            if( $wpvs_license_key_type == 'plugin' ) {
    ...
    public function wpvs_check_customer_license_keys() {
        ...
        if( ! empty($wpvs_account_theme_license) ) {
            if( isset($wpvs_account_theme_license['expires']) && $wpvs_account_theme_license['expires'] <=  $current_time ) {
                $this->check_wpvs_product_license($wpvs_account_theme_license);
            }
    ...
    private function check_wpvs_product_license($wpvs_license) {
        ...
        if( ! isset($wpvs_license['check_update']) || $wpvs_license['check_update'] == false ) {
            ...
            if( $wpvs_license_key_type == 'theme' ) {
                update_option('wpvs_account_theme_license_key', $wpvs_license);
            }
            if( $wpvs_license_key_type == 'plugin' ) {
...
$wpvs_theme_activation_manager = new WPVS_THEME_ACTIVATION_MANAGER();
